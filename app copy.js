...
// console.log(db.ваша_коллекция.find())
// Настройка Express
...
// Настройка Passport
passport.use(new LocalStrategy({ usernameField: 'userlogin' }, (userlogin, password, done) => {
	User.findOne({ userlogin: userlogin })
		.then(user => {
			if (!user) {
				console.log('НЕ верный логин');
				 return done(null, false, { message: 'Incorrect userlogin.' });
...
app.get('/', (req, res) => {
	...
	res.render('register', { messages: null, userIsVerified })
})
...
app.get('/register', (req, res) => {
	console.log('aslkdjfhaslkdjhasldkjhsldfkjsdhflksjdfhlaskdjhsadlf');
...
app.get('/profile', isAuthenticated, async (req, res) => {
	const userIsVerified = req.isAuthenticated();
	res.render('profile', { user: req.user, userIsVerified });
	try {
...
// 	const userIsVerified = req.isAuthenticated();
// 	res.render('login', { userIsVerified })
// });
...
app.post('/updatePhoneNumber', isAuthenticated, async (req, res) => {
	const userIsVerified = req.isAuthenticated();
	try {
		const user = await User.findById(req.user._id);
		if (!user) {
...
app.post('/updateTelegramm', isAuthenticated, async (req, res) => {
	const userIsVerified = req.isAuthenticated();
	try {
		const user = await User.findById(req.user._id);
		if (!user) {
...
app.post('/updateEmail', isAuthenticated, async (req, res) => {
	const userIsVerified = req.isAuthenticated();
	try {
		const user = await User.findById(req.user._id);
		if (!user) {
...
// app.get('/dictionaries', isAuthenticated, async (req, res) => {
// 	const userIsVerified = req.isAuthenticated();
//   try {
// 		const userIsVerified = req.isAuthenticated();
...
// 		if (dictionaries) {
// 			res.render('dictionaries', { dictionaries: dictionaries, userIsVerified });
// 		}
//   } catch (error) {
//     console.error(error);
...
app.get('/dictionaries', isAuthenticated, async (req, res) => {
	try {
		...
		res.render('dictionaries', { dictionaries, userIsVerified });
	} catch (error) {
		console.error(error);
		res.status(500).send('Internal Server Error');
	}
...
// Маршрут для создания нового словаря
// app.post('/createDictionary', isAuthenticated, async (req, res) => {
// 	const userIsVerified = req.isAuthenticated();
//   try {
//     const newDictionary = new Dictionary({ name: req.body.dictionaryName });
...
app.get('/deleteDictionary', isAuthenticated, async (req, res) => {
	const userIsVerified = req.isAuthenticated();
	try {
...
app.get('/editDictionary/:id', isAuthenticated, async (req, res) => {
	const userIsVerified = req.isAuthenticated();
	try {
		...
		if (!dictionary) {
			...
			return res.redirect('/dictionaries', { userIsVerified });
		}
		// Отображаем страницу редактирования словаря с данными о словаре
		res.render('editDictionary', { dictionary, userIsVerified });
	} catch (error) {
		console.error(error);
		req.flash('error', 'Произошла ошибка при загрузке словаря для редактирования');
		res.redirect('/dictionaries', { userIsVerified });
	}
});

app.post('/editDictionary/:id', isAuthenticated, async (req, res) => {
	const userIsVerified = req.isAuthenticated();
	try {
		...
		res.redirect(`/dictionariesList/${dictionaryId}`, { userIsVerified });
	} catch (error) {
		console.error(error);
...
app.post('/updateDictionary/:id', isAuthenticated, async (req, res) => {
	const userIsVerified = req.isAuthenticated();
...
app.get('/dictionariesList/:id', isAuthenticated, async (req, res) => {
	// const currentPage = 'dictionariesList'
	const userIsVerified = req.isAuthenticated();
	try {
		const dictionaryId = req.params.id;
		...
		// console.log(dictionary);
		res.render('dictionariesList', { dictionary, dictionaryId: dictionaryId, userIsVerified }); // Отправляем данные о словаре в шаблон ejs для отображения
	} catch (error) {
		console.error(error);
...
app.post('/deleteWord/:id', isAuthenticated, async (req, res) => {
	const userIsVerified = req.isAuthenticated();
	try {
		...
		res.render('dictionariesList', { dictionary, dictionaryId: '', userIsVerified }); // Отправляем данные о словаре в шаблон ejs для отображения
	} catch (error) {
		console.error(error);
...
app.get('/editWord/:id', isAuthenticated, async (req, res) => {
	const userIsVerified = req.isAuthenticated();
	try {
		const wordId = req.params.id;
		...
		res.render('editWord', { word, dictionaryId, userIsVerified }); // Передаем данные о слове и идентификаторе словаря в шаблон для редактирования
	} catch (error) {
		console.error(error);
...
app.post('/editWord/:id', isAuthenticated, async (req, res) => {
	const userIsVerified = req.isAuthenticated();
	try {
		const wordId = req.params.id;
...
// app.get('/study/:id/easy');   65d75650b5adeac50ba772e8
app.get('/study/:id', isAuthenticated, async (req, res) => {
	const userIsVerified = req.isAuthenticated();
	try {
		const dictionaryId = req.params.id;
		const dictionary = await Dictionary.findById(dictionaryId); // Предположим, что ваша модель называется Dictionary
		...
		res.render('study', { dictionary, dictionaryId: dictionaryId, userIsVerified }); // Отправляем данные о словаре в шаблон ejs для отображения
	} catch (error) {
		console.error(error);
		res.status(500).send('Произошла ошибка при загрузке данных о словаре');
	}
...
app.post('/study/:id', isAuthenticated, async (req, res) => {
	const userIsVerified = req.isAuthenticated();
	try {
		const dictionary = await Dictionary.findById(req.params.id);
		let id = req.params.id
...
app.post('/study/:id/repeated', isAuthenticated, async (req, res) => {
	const userIsVerified = req.isAuthenticated();
	try {
		const wordId = req.params.id;
...
app.get('/admin', isAdmin, async (req, res) => {
  ...
  try {
    ...
    // Рендеринг шаблона с информацией о пользователях
		// const dictionaries = await Dictionary.find({});
    res.render('admin', { users: users, dictionaries: dictionaries, userIsVerified});
  } catch (error) {
...
// 		res.render('admin', { users }); // Отправляем данные о словаре в шаблон ejs для отображения
// } catch (err) {
//     console.error(err);
// }
	
// 	// Здесь можно выполнить необходимые действия для страницы администратора
